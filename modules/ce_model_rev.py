...
class binary_model:  
    def __init__(self, model_path):
        ...
        with self.test_graph.as_default():
            ...
            self.d_rate = self.test_graph.get_tensor_by_name('d_rate:0')
...
#         cam[np.where(cam < 0.5)] = 0
    ...
    def get_softmax(self, image):
        ...
        """
        softmax = self.sess.run(tf.nn.softmax(self.score), feed_dict={self.x: image, self.d_rate: 0.0})
        return softmax
    ...
    def get_gradcam(self, image, class_idx = 1, featuremap_name = 'conv2d_11/Relu:0', desired_cam = 'resized', alpha = 0.3, model_ch = 'bgr'):
        ...
        grads, features = self.sess.run([grad, featuremap], feed_dict={self.x: image, self.d_rate: 0.0})
        cams = []
...
class ensemble_model:  
    def __init__(self, nh_path, nd_path):
        ...
        with self.nh_test_graph.as_default():
            ...
            #print(self.nh_test_graph.get_operations())
            ...
            self.nh_d_rate = self.nh_test_graph.get_tensor_by_name('d_rate:0')
        with self.nd_test_graph.as_default():
            ...
            self.nd_d_rate = self.nd_test_graph.get_tensor_by_name('d_rate:0')
...
#         cam[np.where(cam < 0.7)] = 0 
    ...
    def get_softmax(self, image):
        ...
        image.shape = (None, 512, 512, 3)
        """
        nh_softmax = self.nh_sess.run(tf.nn.softmax(self.nh_score), feed_dict={self.nh_x: image, self.nh_d_rate: 0.0})
        nd_softmax = self.nd_sess.run(tf.nn.softmax(self.nd_score), feed_dict={self.nd_x: image, self.nd_d_rate: 0.0})
